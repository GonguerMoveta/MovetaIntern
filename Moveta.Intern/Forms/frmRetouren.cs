...
namespace Moveta.Intern
{
	...
	/// <summary>
	/// </summary>
	public partial class frmRetouren : SalFormWindow
	{
		...
		/// <returns></returns>
		public static frmRetouren CreateWindow(Control owner)
		{
			frmRetouren frm = new frmRetouren();
			frm.Show(owner);
			return frm;
		}
		...
		/// <returns></returns>
		[DebuggerStepThrough]
		public static frmRetouren FromHandle(SalWindowHandle handle)
		{
			return ((frmRetouren)SalWindow.FromHandle(handle, typeof(frmRetouren)));
		}
		#endregion
...
WHERE prdarztnr = :pnArztNr AND prdrechnr = :pnRechNr")) 
		...
		// FROM pr
		// INTO :frmRetouren.nH, :frmRetouren.nA1, :frmRetouren.nR1,
		// :frmRetouren.nA2, :frmRetouren.nR2,
		// :frmRetouren.nA3, :frmRetouren.nR3,
		// :frmRetouren.nA4, :frmRetouren.nR4,
		// :frmRetouren.nA5, :frmRetouren.nR5,
		// :frmRetouren.nA6, :frmRetouren.nR6
		// WHERE prnr = :pnProzNr')
		// Call SqlFetchNext( phSql, nInd )
		// Set strpSelect = 'SELECT sum(rkdmleis+rkdmarzn+rkdmmahn+rkdmzins+rkmbdmopl-rkzaleis-rkzaarzn-rkzamahn-rkzazins-rkmbzaopl)
		// INTO :pnDMOP
		// FROM pr,rk
		// WHERE rkhalternr = :frmRetouren.nH AND rkkzzahl<2 and  prnr=:pnProzNr and  ('
		// If nR1 != NUMBER_Null
		// Set strpSelect = strpSelect || '(pra1 = :frmRetouren.nA1 AND prr1 = :frmRetouren.nR1) OR '
		// If nR2 != NUMBER_Null
		// Set strpSelect = strpSelect || '(pra2 = :frmRetouren.nA2 AND prr2 = :frmRetouren.nR2) OR '
		// If nR3 != NUMBER_Null
		// Set strpSelect = strpSelect || '(pra3 = :frmRetouren.nA3 AND prr3 = :frmRetouren.nR3) OR '
		// If nR4 != NUMBER_Null
		// Set strpSelect = strpSelect || '(pra4 = :frmRetouren.nA4 AND prr4 = :frmRetouren.nR4) OR '
		// If nR5 != NUMBER_Null
		// Set strpSelect = strpSelect || '(pra5 = :frmRetouren.nA5 AND prr5 = :frmRetouren.nR5) OR '
		// If nR6 != NUMBER_Null
		// Set strpSelect = strpSelect || '(pra6 = :frmRetouren.nA6 AND prr6 = :frmRetouren.nR6) OR '
...
WHERE prnr = :pnProzNr")) 
		...
		/// <param name="pnHalterNr"></param>
		/// <param name="pnRechNr"></param>
		/// <param name="pnExportNr">31.07.23 Ä2126 + , fiexportnr</param>
		/// <returns></returns>
...
VALUES
( :pnArztNr, :pnHalterNr, :pnRechNr, :pnKZZahl, :pdtBuchDat, :pnBelNr, :pnBuchDM, :pnProzNr, 'Retoure', :pnRZNr, SYSDATETIME )")) 
				{
					locals.phSql.Commit();
				}
		...
		/// <returns></returns>
		public SalNumber InsertKonto(SalSqlHandle phSql, SalNumber pnArztNr, SalNumber pnHalterNr, SalNumber pnRechNr, SalString pstrBuchText, SalNumber pnBelNr, SalDateTime pdtBuchDat, SalNumber pnZahlLeis, SalNumber pnZahlArzn, SalNumber pnZahlMahn, SalNumber pnZahlZins, SalNumber pnRPMwSt, SalNumber pnMwSatz, SalNumber pnKZZahl, SalNumber pnDummyHaben)
		{
			// String: pstrKontoCode	! 27.11.23 Ä2144
...
:pnRPMwSt, :pnMwSatz, :pnKZZahl, 0, 0, 0, :pnDummyHaben )")) 
				{
					locals.phSql.Commit();
				}
...
WHERE ROWID = :strROWIDLocal")) 
		...
		/// <param name="nRZNr"></param>
		/// <returns></returns>
		public SalString RetoureBuchen(
					SalNumber nArztNr, 
					SalNumber nHalterNr, 
					SalNumber nRechNr, 
					SalNumber nProzNrX, 
					SalNumber nZahlLeisX, 
					SalNumber nZahlArznX, 
					SalNumber nZahlMahnX, 
					SalNumber nZahlMahnTKX, 
					SalNumber nZahlZinsX, 
					SalNumber nZahlMBKoX, 
					SalNumber nMwStX, 
					SalDateTime dtBuchDatX, 
					SalString strBankKontoX, 
					SalNumber nBelNrX, 
					SalDateTime dtVerjDatX, 
					SalDateTime dtTeilzDatX, 
					SalNumber nRZNr)
		{
			#region Local Variables
			...
			using (new SalContext(this))
			{
				...
				strBuchText = "Retoure " + Int.PalStrNum(nArztNr, 4, 0) + "/R" + Int.PalStrNum(nRechNr, 6, 0);
				// 14.04.14 Ä1046
				...
				InsertZahlung(hSqlBankZahl, nArztNr, nHalterNr, nRechNr, nProzNr, nKZZahl, nBelNrX, nZahlLeisX + nZahlArznX + nZahlMahnX + nZahlZinsX + nZahlMBKoX, dtBuchDatX, nRZNr);
				...
				{
					// 20.09.04 nZahlMahnTKX statt nZahlMahnX
					nDiverse = nZahlLeisX + nZahlArznX + nZahlMahnTKX + nZahlZinsX + nZahlMBKoX;
				}
				InsertFiBu(hSqlBankZahl, 0, strBankKontoX, 16000, nBelNrX, dtBuchDatX, strBuchText, nDiverse, 3, nArztNr, nHalterNr, nRechNr, dfExportNr.Number);
				// 20.09.04 nach Fehlerhinweis von Petra
				// 15.09.14 F2080
				// Set nDiverse = nZahlMahnX - nZahlMahnTKX
				// Call InsertFiBu( hSqlBankZahl, 0, strBankKontoX, frmMain.nErloeskonto, nBelNrX, dtBuchDatX, strBuchText, nDiverse, 3, nArztNr, nHalterNr, nRechNr )
				Var.nCheckMwSt = Int.PalCheckAuslMwSt(nArztNr);
				nDiverse = ((nZahlMahnX - nZahlMahnTKX) * 10000 / (100 + Var.nCheckMwSt)).Round() / 100;
				if (InsertFiBu(hSqlBankZahl, 0, strBankKontoX, App.frmMain.nErloeskonto, nBelNrX, dtBuchDatX, strBuchText, nDiverse, 3, nArztNr, nHalterNr, nRechNr, dfExportNr.Number)) 
				{
				}
				nDiverse = (nZahlMahnX - nZahlMahnTKX) - nDiverse;
				// 30.06.20 Ä1892
				...
				if (InsertFiBu(hSqlBankZahl, 0, strBankKontoX, Var.nMwStSenkung, nBelNrX, dtBuchDatX, strBuchText, nDiverse, 3, nArztNr, nHalterNr, nRechNr, dfExportNr.Number)) 
				{
				}
				...
				if (nRechNr >= 999990) 
				{
					...
					if (nZahlMBKoX < 0) 
					{
						strBuchTextDummy = "MBc" + Int.PalStrNum(nArztNr, 4, 0) + "/" + Int.PalStrNum(nHalterNr, 5, 0) + "/" + Int.PalStrNum(nRechNr, 6, 0);
						nDiverse = nZahlMBKoX;
						InsertFiBu(hSqlBankZahl, 0, strBankKontoX, 16510, nBelNrX, dtBuchDatX, strBuchTextDummy, nDiverse, 3, nArztNr, nHalterNr, nRechNr, dfExportNr.Number);
					}
				...
				Var.nDummyHaben = nZahlLeisX + nZahlArznX + nZahlMahnTKX + nZahlZinsX;
				// 11.11.05 #8
				if (nRechNr >= 999990) 
				{
					nRPMwSt = (nZahlLeisX * nMwStX * 100 / (100 + nMwStX)).Round() / 100;
				}
				// Else ! 18.12.12 F1477 auskommentiert
				// Set nRPMwSt = SalNumberRound((nZahlLeisX+nZahlArznX) * nMwStX * 100 / (100 + nMwStX)) /100
				// Call InsertKonto( hSqlBankZahl, nArztNr, nHalterNr, nRechNr, strBuchText, nBelNrX,
				// dtBuchDatX, nZahlLeisX, nZahlArznX, nZahlMahnX+nZahlMBKoX, nZahlZinsX, nRPMwSt, nMwStX, 1, nDummyHaben )
				// 18.12.12 F1477
				// Call InsertKonto( hSqlBankZahl, nArztNr, nHalterNr, nRechNr, strBuchText, nBelNrX,
				// dtBuchDatX, nZahlLeisX, nZahlArznX, nZahlMahnTKX, nZahlZinsX, nRPMwSt, nMwStX, 1, nDummyHaben )
				// 18.03.13 F1597 +  or (nZahlArznX = 0 and nZahlLeisX = 0)
				if (nZahlLeisX != 0 || (nZahlArznX == 0 && nZahlLeisX == 0)) 
				{
					// 18.12.12 F1477
					if (nRechNr < 999990) 
					{
					...
					// 09.01.13 F1477 nDummyHaben -> nZahlLeisX
					// 22.03.13 F1612 nZahlLeisX -> nDummyHaben
					Var.nDummyHaben = nZahlLeisX + nZahlMahnTKX + nZahlZinsX;
					// 12.09.13 F1704 auskommentiert
					// Set nZahlMahnTKX = 0
					// Set nZahlZinsX = 0

					InsertKonto(hSqlBankZahl, nArztNr, nHalterNr, nRechNr, strBuchText, nBelNrX, dtBuchDatX, nZahlLeisX, 0, nZahlMahnTKX, nZahlZinsX, nRPMwSt, nMwStX, 1, Var.nDummyHaben);
					// 21.10.13 F1704 auskommentiert
					nZahlMahnTKX = 0;
					nZahlZinsX = 0;
				...
				if (nZahlArznX != 0) 
				{
					...
					// 09.01.13 F1477 nDummyHaben -> nZahlArznX
					// 22.03.13 F1612 nZahlArznX -> nDummyHaben
					Var.nDummyHaben = nZahlArznX + nZahlMahnTKX + nZahlZinsX;
					// 12.09.13 F1704 auskommentiert
					// Set nZahlMahnTKX = 0
					// Set nZahlZinsX = 0

					InsertKonto(hSqlBankZahl, nArztNr, nHalterNr, nRechNr, strBuchText, nBelNrX, dtBuchDatX, 0, nZahlArznX, nZahlMahnTKX, nZahlZinsX, nRPMwSt, nMwStX, 1, Var.nDummyHaben);
					// 21.10.13 F1704 auskommentiert
					nZahlMahnTKX = 0;
					nZahlZinsX = 0;
		...
		/// <summary>
		/// frmRetouren WindowActions Handler
		/// </summary>
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void frmRetouren_WindowActions(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			switch (e.ActionType)
			{
				case Sys.SAM_Create:
					this.frmRetouren_OnSAM_Create(sender, e);
					break;
				
				case Sys.SAM_Close:
					this.frmRetouren_OnSAM_Close(sender, e);
					break;
			}
			#endregion
		}
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void frmRetouren_OnSAM_Create(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
			...
			// 06.04.23 2 -> 7 laut HE
			this.lbBank.SetListSelectedIndex(7);
		...
		/// <param name="sender"></param>
		/// <param name="e"></param>
		private void frmRetouren_OnSAM_Close(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
...
into :frmRetouren.dfArztName, :frmRetouren.nArztNr2
from A where aarztnr=:frmRetouren.dfArztNr");
...
into :frmRetouren.dfHalterName
from H where  harztnr = :frmRetouren.dfArztNr AND hhalternr = :frmRetouren.dfHalterNr AND hdeaktiv IS NULL ");
			...
			// INTO
			// :frmRetouren.dfRechNr,
			// :frmRetouren.dfRechDat,
			// :frmRetouren.dfDMLeis,
			// :frmRetouren.dfDMArzn,
			// :frmRetouren.dfDMMahn,
			// :frmRetouren.dfDMZins,
			// :frmRetouren.dfDMRest,
			// :frmRetouren.dfMS
			// FROM RK
			// WHERE rkarztnr IN (:frmRetouren.nArztNr, :frmRetouren.nArztNr2) AND rkhalternr = :frmRetouren.dfHalterNr AND
...
INTO :frmRetouren.dfHalterNr,
:frmRetouren.dfDMLeis,
:frmRetouren.dfDMArzn,
:frmRetouren.dfDMMahn,
:frmRetouren.dfDMMBKo,
:frmRetouren.dfDMZins,
:frmRetouren.dfVerjDat
FROM RK
WHERE rkarztnr = :frmRetouren.dfArztNr
AND rkrechnr = :frmRetouren.dfRechNr AND rkdeaktiv IS NULL ");
				}
				else
				{
					Int.SqlImmedSel(@"SELECT rkhalternr, rkdmleis, rkdmarzn, rkdmmahn, rkmbdmopl, rkdmzins, rkverjaehr
...
FROM RK
WHERE rkarztnr = :frmRetouren.dfArztNr
AND rkhalternr = :frmRetouren.dfHalterNr
AND rkrechnr = :frmRetouren.dfRechNr AND rkdeaktiv IS NULL ");
				}
				Int.SqlImmedSel(@"SELECT hti || hvn || ' ' || hnn
into :frmRetouren.dfHalterName
from H where  harztnr = :frmRetouren.dfArztNr AND hhalternr = :frmRetouren.dfHalterNr  AND hdeaktiv IS NULL");
				...
				Int.SqlIstDa(@" FROM Z WHERE zhalternr=:frmRetouren.dfHalterNr
and " + Int.PalArztNr("z", this.nArztNr, this.nArztNr2) + @" and zrechnr=:frmRetouren.dfRechNr
AND zart not in ('HS', 'ZM') AND zdeaktiv IS NULL", ref temp1);
			...
			if (this.dfRechNr.Number >= 999990) 
			{
				...
				this.nZCount = 0;
				// 06.01.13 Ä1005
				// Call SqlImmedSel('SELECT count(*) INTO :frmRetouren.nZCount FROM prd WHERE prdarztnr = :frmRetouren.nArztNr AND prdhalternr = :frmRetouren.dfHalterNr')
				this.nZCount = Int.SqlGetAnzahl("FROM prd WHERE prdarztnr = :frmRetouren.nArztNr AND prdhalternr = :frmRetouren.dfHalterNr");
				if (this.nZCount > 0) 
				{
					if (this.cmbProzNr.Text == "") 
					{
						this.cmbProzNr.ClearList();
						this.cmbProzNr.PopulateList(this.hSqlZa, "SELECT DISTINCT prdnr FROM prd WHERE prdarztnr = :frmRetouren.nArztNr AND prdhalternr = :frmRetouren.dfHalterNr ORDER BY prdnr DESC ");
...
Ist dies in Ordnung ?", "Achtung", (Sys.MB_IconQuestion | Sys.MB_YesNo))) 
			...
			this.nZCount = 0;
			// 06.01.13 Ä1005
			// Call SqlImmedSel('SELECT count(*) INTO :frmRetouren.nZCount FROM rzp WHERE rzparztnr IN ( :frmRetouren.nArztNr, :frmRetouren.nArztNr2)
			// AND rzphalternr = :frmRetouren.dfHalterNr AND rzprechnr = :frmRetouren.dfRechNr')
			this.nZCount = Int.SqlGetAnzahl(@"FROM rzp WHERE rzparztnr IN ( :frmRetouren.nArztNr, :frmRetouren.nArztNr2)
AND rzphalternr = :frmRetouren.dfHalterNr AND rzprechnr = :frmRetouren.dfRechNr");
			if (this.nZCount >= 1) 
			{
				if (this.dfRZNr.Number == Sys.NUMBER_Null) 
				{
					Int.SqlImmedSel(@"SELECT rzpnr INTO :frmRetouren.dfRZNr FROM rzp WHERE rzparztnr IN ( :frmRetouren.nArztNr, :frmRetouren.nArztNr2)
AND rzphalternr = :frmRetouren.dfHalterNr AND rzprechnr = :frmRetouren.dfRechNr ORDER BY rzpnr DESC");
				}
			}
			#endregion
		}
...
from pr where prnr=:frmRetouren.nProzNr");
		...
		private void dfMatch_OnSAM_Validate(object sender, WindowActionsEventArgs e)
		{
			...
			tblHalter.CreateWindow(this, "frmRetouren", this.dfArztNr.Number, 0, this.dfMatch.Text, "", "", "", "");
		...
		private void cbZettel_OnSAM_Click(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
			frmZettel.CreateWindow(this, "frmRetouren", this.dfArztNr.Number, this.dfHalterNr.Number, this.dfRechNr.Number);
			#endregion
		}
		...
		private void pbOK_OnSAM_Click(object sender, WindowActionsEventArgs e)
		{
			#region Actions
			e.Handled = true;
			if (this.dfDMMahnTK.IsEmpty()) 
			{
				Sal.MessageBox(@"Bitte erst ""€-Mahn TK"" eingeben.

Buchung wurde NICHT durchgeführt!", "Achtung", (Sys.MB_Ok | Sys.MB_IconStop));
			}
			else
			{
...
Ebenso muß die Buchung für jeden %-Satz einzeln durchgeführt werden.", "Achtung", (Sys.MB_Ok | Sys.MB_IconExclamation));
				...
				else
				{
					...
					this.strErgebnis = this.RetoureBuchen(this.dfArztNr.Number, this.dfHalterNr.Number, this.dfRechNr.Number, this.nProzNr, this.dfDMLeis.Number * -1, this.dfDMArzn.Number * -1, this.dfDMMahn.Number * -1, this.dfDMMahnTK.Number * -1, this.dfDMZins.Number * 
						-1, this.dfDMMBKo.Number * -1, this.dfMwSt.Number, this.dfBuchDat.DateTime, this.strBankKonto, this.dfBelNr.Number, this.dfVerjDat.DateTime, this.dfTeilzDat.DateTime, this.dfRZNr.Number);
					this.dfDMMahnTK.Number = Sys.NUMBER_Null;
				}
			}
			#endregion
		}
		...
		private void dfRZNr_OnSAM_Validate(object sender, WindowActionsEventArgs e)
		{
			...
			e.Handled = true;
			Sal.WaitCursor(true);
			Int.SqlImmedSel("SELECT rzarztnr, rzhalternr INTO :frmRetouren.dfArztNr, :frmRetouren.dfHalterNr FROM rz WHERE rznr = :frmRetouren.dfRZNr AND rzdeaktiv IS NULL");
