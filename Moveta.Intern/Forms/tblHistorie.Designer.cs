// <ppj name="Moveta.Intern" date="1/17/2024 7:59:41 AM" id="F4EC85BAD2BF79AC25C9F8643540E90F9BE1DAF0"/>
// ======================================================================================================
// This code was generated by the Ice Porter(tm) Tool version 4.8.15.0
// Ice Porter is part of The Porting Project (PPJ) by Ice Tea Group, LLC.
// The generated code is not guaranteed to be accurate and to compile without
// manual modifications.
// 
// ICE TEA GROUP LLC SHALL IN NO EVENT BE LIABLE FOR ANY DAMAGES WHATSOEVER
// (INCLUDING, WITHOUT LIMITATION, DAMAGES FOR LOSS OF BUSINESS PROFITS, BUSINESS
// INTERRUPTION, LOSS OF BUSINESS INFORMATION, OR ANY OTHER LOSS OF ANY KIND)
// ARISING OUT OF THE USE OR INABILITY TO USE THE GENERATED CODE, WHETHER
// DIRECT, INDIRECT, INCIDENTAL, CONSEQUENTIAL, SPECIAL OR OTHERWISE, REGARDLESS
// OF THE FORM OF ACTION, EVEN IF ICE TEA GROUP LLC HAS BEEN ADVISED OF THE
// POSSIBILITY OF SUCH DAMAGES.
// =====================================================================================================
using System;
using System.Text;
using System.Drawing;
using System.Diagnostics;
using System.Collections;
using System.Windows.Forms;
using System.ComponentModel;
using MT;
using PPJ.Runtime;
using PPJ.Runtime.Com;
using PPJ.Runtime.Sql;
using PPJ.Runtime.Vis;
using PPJ.Runtime.Windows;
using PPJ.Runtime.Windows.QO;
using PPJ.Runtime.XSal;

namespace Moveta.Intern
{
	
	public partial class tblHistorie
	{
		
		/// <summary>
		/// Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;
		
		#region Window Accessories
		
		/// <summary>
		/// Toolbar control.
		/// </summary>
		protected SalFormToolBar ToolBar;
		
		/// <summary>
		/// StatusBar control.
		/// </summary>
		protected SalFormStatusBar StatusBar;
		#endregion
		
		
		#region Window Controls
		public SalPushbutton pbAbbruch;
		public SalPushbutton pbDruck;
		public SalTableColumn colDatum;
		public SalTableColumn colRgNr;
		public SalTableColumn colTyp;
		public SalTableColumn colBez;
		public SalTableColumn colDM;
		#endregion
		
		#region Windows Form Designer generated code
		
		/// <summary>
		/// Required method for Designer support - do not modify
		/// the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			this.ToolBar = new PPJ.Runtime.Windows.SalFormToolBar();
			this.StatusBar = new PPJ.Runtime.Windows.SalFormStatusBar();
			this.pbAbbruch = new PPJ.Runtime.Windows.SalPushbutton();
			this.pbDruck = new PPJ.Runtime.Windows.SalPushbutton();
			this.colDatum = new PPJ.Runtime.Windows.SalTableColumn();
			this.colRgNr = new PPJ.Runtime.Windows.SalTableColumn();
			this.colTyp = new PPJ.Runtime.Windows.SalTableColumn();
			this.colBez = new PPJ.Runtime.Windows.SalTableColumn();
			this.colDM = new PPJ.Runtime.Windows.SalTableColumn();
			this.ToolBar.SuspendLayout();
			this.SuspendLayout();
			// 
			// ToolBar
			// 
			this.ToolBar.Name = "ToolBar";
			this.ToolBar.Font = new Font("Tahoma", 8f, System.Drawing.FontStyle.Regular);
			this.ToolBar.Height = 42;
			this.ToolBar.TabStop = true;
			this.ToolBar.Visible = true;
			this.ToolBar.Create = true;
			this.ToolBar.Controls.Add(this.pbDruck);
			this.ToolBar.Controls.Add(this.pbAbbruch);
			// 
			// StatusBar
			// 
			this.StatusBar.Name = "StatusBar";
			this.StatusBar.Visible = true;
			// 
			// pbAbbruch
			// 
			this.pbAbbruch.Name = "pbAbbruch";
			this.pbAbbruch.Font = new Font("Tahoma", 9f, System.Drawing.FontStyle.Bold);
			this.pbAbbruch.AcceleratorKey = Keys.Escape;
			this.pbAbbruch.Text = "Abbruch";
			this.pbAbbruch.Location = new System.Drawing.Point(-25, -42);
			this.pbAbbruch.Size = new System.Drawing.Size(25, 42);
			this.pbAbbruch.TextImageRelation = System.Windows.Forms.TextImageRelation.ImageAboveText;
			this.pbAbbruch.WindowActions += new PPJ.Runtime.Windows.WindowActionsEventHandler(this.pbAbbruch_WindowActions);
			this.pbAbbruch.TabIndex = 0;
			// 
			// pbDruck
			// 
			this.pbDruck.Name = "pbDruck";
			this.pbDruck.Font = new Font("Tahoma", 9f, System.Drawing.FontStyle.Bold);
			this.pbDruck.Location = new System.Drawing.Point(4, -1);
			this.pbDruck.Size = new System.Drawing.Size(45, 42);
			this.pbDruck.TransparentColor = System.Drawing.Color.White;
			this.pbDruck.Image = global::Moveta.Intern.Properties.Resources.printer;
			this.pbDruck.TextImageRelation = System.Windows.Forms.TextImageRelation.ImageAboveText;
			this.pbDruck.WindowActions += new PPJ.Runtime.Windows.WindowActionsEventHandler(this.pbDruck_WindowActions);
			this.pbDruck.TabIndex = 1;
			// 
			// colDatum
			// 
			this.colDatum.Name = "colDatum";
			this.colDatum.Title = "Datum";
			this.colDatum.DataType = PPJ.Runtime.Windows.DataType.DateTime;
			this.colDatum.Enabled = false;
			this.colDatum.Format = "dd.MM.yy";
			this.colDatum.TabIndex = 0;
			// 
			// colRgNr
			// 
			this.colRgNr.Name = "colRgNr";
			this.colRgNr.Title = "Rechnung";
			this.colRgNr.Width = 58;
			this.colRgNr.DataType = PPJ.Runtime.Windows.DataType.Number;
			this.colRgNr.Enabled = false;
			this.colRgNr.Format = "#0";
			this.colRgNr.TextAlign = System.Windows.Forms.HorizontalAlignment.Right;
			this.colRgNr.TabIndex = 1;
			// 
			// colTyp
			// 
			this.colTyp.Name = "colTyp";
			this.colTyp.Title = "Typ";
			this.colTyp.Enabled = false;
			this.colTyp.TabIndex = 2;
			// 
			// colBez
			// 
			this.colBez.Name = "colBez";
			this.colBez.Title = "Bezeichnung";
			this.colBez.Width = 779;
			this.colBez.MaxLength = 32000;
			this.colBez.DataType = PPJ.Runtime.Windows.DataType.String;
			this.colBez.Enabled = false;
			this.colBez.TabIndex = 3;
			// 
			// colDM
			// 
			this.colDM.Name = "colDM";
			this.colDM.Title = "€";
			this.colDM.DataType = PPJ.Runtime.Windows.DataType.Number;
			this.colDM.Enabled = false;
			this.colDM.Format = "#,##0.00";
			this.colDM.TextAlign = System.Windows.Forms.HorizontalAlignment.Right;
			this.colDM.TabIndex = 4;
			// 
			// tblHistorie
			// 
			this.Controls.Add(this.colDatum);
			this.Controls.Add(this.colRgNr);
			this.Controls.Add(this.colTyp);
			this.Controls.Add(this.colBez);
			this.Controls.Add(this.colDM);
			this.Controls.Add(this.ToolBar);
			this.Controls.Add(this.StatusBar);
			this.Name = "tblHistorie";
			this.ClientSize = new System.Drawing.Size(1130, 739);
			this.Font = new Font("Tahoma", 9f, System.Drawing.FontStyle.Regular);
			this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
			this.Text = "Historie";
			this.Location = new System.Drawing.Point(66, 76);
			this.StartPosition = System.Windows.Forms.FormStartPosition.Manual;
			this.UseVisualStyles = true;
			this.WindowActions += new PPJ.Runtime.Windows.WindowActionsEventHandler(this.tblHistorie_WindowActions);
			this.ToolBar.ResumeLayout(false);
			this.ResumeLayout(false);
		}
		#endregion
		
		#region System Methods/Properties
		
		/// <summary>
		/// Release global reference.
		/// </summary>
		/// <param name="disposing"></param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && components != null) 
			{
				components.Dispose();
			}
			if (disposing && App.tblHistorie == this) 
			{
				App.tblHistorie = null;
			}
			base.Dispose(disposing);
		}
		#endregion
	}
}
