...
{
	/// <summary>
	/// An SpTranslator for translating stored procedures to SqlServer syntax
	/// </summary>
	public class SpTranslatorSqlServer: SpTranslator
    {
                            ...
                            foreach (TreeNode child in parentNode.Children)
                            {
                                switch (child.NodeType)
                                {
                                    case Tokens.DataType:
                                        dataType = SpDictionarySqlServer.GetValue(child.NodeValue);
                                ...
                                foreach (TreeNode child in parentNode.Children)
                                {
                                    switch (child.NodeType)
                                    {
                                        ...
                                        case Tokens.Identifier:
                                            ...
                                            if (dataType != "cursor")
                                            {
                                                result.Append("DECLARE @" + child.NodeValue + " ");
                                                result.AppendLine(dataType);
                                            }
                                            else
                                            {
            ...
            foreach (TreeNode node in blockNode.Children)
            {
                switch (node.NodeType)
                {
                    ...
                    case Tokens.Statement:
                        ...
                        {
                            case "BREAK": //in T-Sql we can't add an identifier after BREAK
                                ...
                                foreach (TreeNode child in node.Children)
                                {
                                    switch (child.NodeType)
                                    {
                                        case Tokens.Expression:
                                            ...
                                            //check if we have a condition of the form: @var = NULL; if yes then we have to replace it with @var IS NULL
                                            if(expression.Contains("=") && ContainsWord(expression, "NULL"))
                                            {
                                                expression = expression.Replace("=", " IS ");
                                            }
                                            block.AppendLine(expression);
                                            //if we have a function inside an IF then we have to create a condition
                                            //because we don't have a boolean return value anymore (like the Sql* functions have)
                                            if (containsFunction)
                                            {
                                                if (sqlFetchParam.Count > 0)
                                                {
                                                    condition = sqlFetchParam.Dequeue() + " = 0 \r\n";
                                                }
                                                else
                                                {
                                                    condition = "@@ERROR = 0 \r\n";
                            ...
                            case "WHILE":
                                ...
                                if (sqlFetch.Count > 0)
                                {
                                    ...
                                    block.AppendLine(SpDictionarySqlServer.GetValue("SQLFETCH", sqlFetchParam.Dequeue()));
                                    block.AppendLine("END");
                                }
                                else
                                {
                            ...
                            case "ON PROCEDURE":
                                switch (node.Children[1].NodeValue)
                                {
                                    ...
                                    case "FETCH":
                                        ...
                                        block.AppendLine("SET @returnValue = 0");
        ...
        private string TranslateFunction(TreeNode node)
        {
            ...
            switch (functionName)
            {
                ...
                case "SQLCOMMIT":
                case "SQLERROR":
                    function.AppendLine(SpDictionarySqlServer.GetValue(functionName));
                    break;

                case "SQLCLOSE":
                    cursors[parameters[0]].Status = CursorStatus.Deallocated;
                    function.AppendLine(SpDictionarySqlServer.GetValue(functionName, parameters[0]));
                    break;
                ...
                case "SQLSETLOCKTIMEOUT":
                    function.AppendLine(SpDictionarySqlServer.GetValue(functionName, parameters[1]));
                    break;

                case "SQLEXISTS":
                    function.AppendLine(SpDictionarySqlServer.GetValue(functionName, parameters[0], parameters[1]));
                    break;

                case "SQLFETCHNEXT":
                case "SQLFETCHPREVIOUS":
                    function.Append(SpDictionarySqlServer.GetValue(functionName, parameters[0]));
                    ...
                    function.AppendLine(SpDictionarySqlServer.GetValue("SQLFETCH", parameters[1]));
                    break;

                case "SQLFETCHROW":
                    function.Append(SpDictionarySqlServer.GetValue(functionName, parameters[0], parameters[1]));
                    function.AppendLine(" " + cursors[parameters[0]].Sql.Into);
                    //store the fetch for later use in WHILE
                    sqlFetch.Enqueue(function.ToString());
                    sqlFetchParam.Enqueue(parameters[2]);
                    //append the set statement
                    function.AppendLine(SpDictionarySqlServer.GetValue("SQLFETCH", parameters[2]));
                    break;

                case "SQLGETERRORTEXT":
                    function.AppendLine(SpDictionarySqlServer.GetValue(functionName, parameters[1], parameters[0]));
                    break;
                ...
                case "SQLPREPARE":
                    if (isQuery)
                    {
                        if (cursors[parameters[0]].Status == CursorStatus.Declared || cursors[parameters[0]].Status == CursorStatus.Opened)
                        {
                            ...
                            function.AppendLine("DEALLOCATE " + parameters[0]);
                        }
                        function.AppendLine(SpDictionarySqlServer.GetValue(functionName, parameters[0], parameters[1]));
                ...
                case "SQLRETRIEVE":
                    ...
                    //function.AppendLine(SpDictionarySqlServer.GetValue("SQLRETRIEVE", parameters[1], parameters[2]));
                    break;
                ...
                case "SQLGETPARAMETER":
                    switch (parameters[1])
                    {
                        case "DBP_VERSION":
                        case "4":
                            function.AppendLine(SpDictionarySqlServer.GetValue("SQLGETPARAMETER2", parameters[2]));
                            break;

                        case "DBP_LOCKWAITTIMEOUT":
                        case "5":
                            function.AppendLine(SpDictionarySqlServer.GetValue("SQLGETPARAMETER1", parameters[2]));
                            break;
                ...
                case "SQLSETPARAMETER":
                    switch (parameters[1])
                    {
                        case "DBP_LOCKWAITTIMEOUT":
                        case "5":
                            function.AppendLine(SpDictionarySqlServer.GetValue("SQLSETPARAMETER", parameters[2]));
                            break;
                ...
                case "SQLSETISOLATIONLEVEL":
                    ...
                    function.AppendLine(SpDictionarySqlServer.GetValue("SQLSETISOLATIONLEVEL", isoLevel));
                    break;

                case "SQLSTORE":
                    if (isQuery)
                    {
                        function.AppendLine(SpDictionarySqlServer.GetValue("SQLSTORE1", parameters[0], parameters[1]));
                    }
                    else
                    {
                        function.AppendLine(SpDictionarySqlServer.GetValue("SQLSTORE2", parameters[1]));
                    }
                    break;
            }
            ...
            foreach (TreeNode node in exprNode.Children)
            {
                switch (node.NodeType)
                {
                    ...
                    case Tokens.SysKeyword:
                        string keyword = SpDictionarySqlServer.GetValue(node.NodeValue);
                    ...
                    case Tokens.Identifier:
                        if (cursors.ContainsKey(node.NodeValue))
                        {
                            expr.Append(node.NodeValue);
                        }
                        else
                    ...
                    case Tokens.Function:
                        ...
                        expr.AppendLine(TranslateFunction(node));
                        break;
                    ...
                    case Tokens.StringConst:
                        ...
                        if (reg.IsMatch(node.NodeValue))
                        {
                            ...
                            string sqlStatement = sqlTrans.TranslateSql(node.NodeValue.Substring(1, node.NodeValue.Length - 2));
        ...
        private string CheckForSimpleOnProcedureFetch()
        {
            ...
            if (reg.IsMatch(Source))
            {
                ...
                {
                    SqlTranslator.SqlTranslator sqlTrans = new fecher.SqlTranslator.SqlTranslator();
                    sqlStatement = sqlTrans.TranslateSql(match.Groups[1].Value);
                    sqlStatement = sqlStatement.Replace(":", "@");
