...
namespace fecher.SqlTranslator
{
    ...
    /// </summary>
    public class SyntacticAnalyzerSqlBase : ISyntacticAnalyzer
    {
        private IToken token = new Token();
        private Tree syntaxTree = new Tree(new TreeNode(Tokens.Root));
        private LexicalAnalyzerSqlBase lex = new LexicalAnalyzerSqlBase();
        ...
        public Tree Parse(string sourceSql)
        {
            ...
            if (!sourceSql.Contains(DatabaseSettings.Default.Separator))
            {
                ...
                switch (token.LiteralValue)
                {
                    ...
                    case "CHECK": CheckStatement(syntaxTree.root); break;
        ...
        {
            // CREATE TABLE table ( {col data_type [NOT NULL| NOT NULL WITH DEFAULT | NOT NULL AUTO_INCREMENT(SEED,STEP)]}*
            // [PRIMARY KEY ( {col[,]}* )]
            ...
            TreeNode node;
            TreeNode temp = new TreeNode(Tokens.Keyword, "CREATE TABLE");
        ...
        {
            TreeNode node;
            //in SqlBase you can have a comma before a join and the statement is still valid
            //check this case and ignore comma for sql server
            ...
            do
            {
                ...
                if (token.LexicalCode == Tokens.Keyword) //join specification
                {
                    HandleJoin(token, parent, node);
                }
                ...
                if (token.LexicalCode == Tokens.Comma)
                {
                    ...
                    if (token.LexicalCode == Tokens.Keyword && token.LiteralValue == "JOIN")
                    {
                        HandleJoin(token, parent, node);
                    }
                    else
        ...
        void HandleJoin(IToken token, TreeNode parent, TreeNode node)
        {
            if (token.LiteralValue == "NATURAL" || token.LiteralValue == "INNER" || token.LiteralValue == "CROSS" ||
                token.LiteralValue == "LEFT" || token.LiteralValue == "RIGHT" || token.LiteralValue == "FULL" ||
                token.LiteralValue == "JOIN")
            {
                JoinSpecification(parent);
            }
            else if (token.LiteralValue != "WHERE" && token.LiteralValue != "INTO" &&
                     token.LiteralValue != "ORDER" && token.LiteralValue != "GROUP" &&
                     token.LiteralValue != "HAVING" && token.LiteralValue != "UNION" && token.LiteralValue != "FOR") //correlation
            {
                node.AddChild(new TreeNode(Tokens.Correlation, token.LiteralValue, token.LiteralValue));
                token = lex.GetToken();
            }
        ...
        void OrderBy(TreeNode parent)
        {
            ...
            do
            {
                ...
                else if (token.LexicalCode == Tokens.NumericConst) //constant
                {
                    node = new TreeNode(Tokens.NumericConst, token.LiteralValue);
                    parent.AddChild(node);
                }
        ...
        void Values(TreeNode parent)
        {
            ...
            //VALUES ({constant|bind variable|system keyword}*)[\ value_list] [ADJUSTING cursor]
                ...
                while (token.LexicalCode != Tokens.RightParant)
                {
                    switch (token.LexicalCode)
                    {
                        ...
                        case Tokens.Keyword: //keyword
                            {
                                string tempVal = token.LiteralValue;
                                if (tempVal == "CURRENT")
                                {
                                    //CURRENT DATE|TIME|DATETIME|TIMESTAMP
                                    token = lex.GetToken();
                                    tempVal += " " + token.LiteralValue;
                                }
                                node = new TreeNode(Tokens.Keyword, tempVal, index.ToString());
                                parent.AddChild(node);
                                index++;
                                break;
                            }
            ...
            // [\ value_list]
            //if(token.LexicalCode == Tokens.Backslash)
            //{
            //    node = new TreeNode(Tokens.Backslash, "\\");
            //    parent.AddChild(node);
            //    token = lex.GetToken();
            //}
            ...
            if (token.LiteralValue == "ADJUSTING")
            {
                node = new TreeNode(Tokens.Keyword, "ADJUSTING");
                parent.AddChild(node);
                token = lex.GetToken();
                if (token.LexicalCode == Tokens.Identifier)
                {
        ...
        {
            //SET {column = expr|NULL|(subselect)}*
        ...
        private void UpdateStatistics(TreeNode parent)
        {
            ...
            //INDEX ident [(SET ident = expr [,])* | (DISTINCTCOUNT((ident [,])*) = expr [,])*] }|
            //TABLE ident [(SET ident = expr [,])*] |
            //DATABASE
            ...
            if (token.LiteralValue == "ON")
            {
                node = new TreeNode(Tokens.Keyword, "ON");
                parent.AddChild(node);
            }
            else
            {
            ...
            if (token.LiteralValue == "INDEX" || token.LiteralValue == "TABLE")
            {
                token = lex.GetToken();
                node = new TreeNode(Tokens.Identifier, token.LiteralValue);
                parent.AddChild(node);
                ...
                if (token.LiteralValue == "SET")
                {
                    ...
                    while (token.LexicalCode != Tokens.Null)
                    {
                        token = lex.GetToken();
                        node = new TreeNode(Tokens.Identifier, token.LiteralValue);
                        parent.AddChild(node);
                ...
                //(DISTINCTCOUNT((ident [,])*) = expr [,])*
                else if (token.LiteralValue == "DISTINCTCOUNT")
                {
                    while (token.LexicalCode != Tokens.Null)
                    {
                        node = new TreeNode(Tokens.Keyword, "DISTINCTCOUNT");
                        parent.AddChild(node);
                        ...
                        if (token.LexicalCode == Tokens.RelatOperator)
                        {
                            token = lex.GetToken();
                            node = new TreeNode(Tokens.RelatOperator, "=");
                            parent.AddChild(node);
            ...
            else if (token.LiteralValue == "DATABASE")
            {
                token = lex.GetToken();
            }
            else
                ...
                && token.LiteralValue != "WHERE")
            {
                ...
                if (token.LexicalCode == Tokens.Keyword)
                {
                    if ((region == ExpressionRegion.Select && !lex.IsKeywordBreakingSELECT(token.LiteralValue) && token.LiteralValue.ToUpper() != "AS")
                        ...
                        ((token.LiteralValue == "ORDER" || token.LiteralValue == "GROUP") && lex.Peek().LiteralValue != "BY")) //Hack for cases where KEY is a column name
                    {
                        token.LexicalCode = Tokens.Identifier;
                    }
                    else
                ...
                switch (token.LexicalCode)
                {
                    ...
                    case Tokens.LeftParant: //expression
                        {
                            //we need to encapsulate everything between () as a expression
                            if (parent.NodeType == Tokens.Expression &&
        ...
        void SearchCondition(TreeNode parent)
        {
                   ...
                   token.LexicalCode != Tokens.Null)
            {
                ...
                if (subSelect.Count > 0 &&
                    syntaxTree.FindAll(parent, ")") > syntaxTree.FindAll(parent, "("))
                {
        ...
        void Predicate(TreeNode parent)
        {
            ...
            else
            {
                ...
                if (token.LexicalCode == Tokens.RelatOperator)//relational operator
                {
                    ...
                    parent.AddChild(node);
                    token = lex.GetToken();
                    if (token.LexicalCode == Tokens.Keyword)//[ANY/SOME|ALL]
                    {
                    ...
                    else
                    {
                        ...
                        else if (token.LiteralValue != "LEFT" && token.LiteralValue != "RIGHT")
                        {
                            ...
                            {
                                parent.AddChild(new TreeNode(Tokens.JoinOperator, "", "INNER"));
                                parent.NodeInfo = "INNER JOIN";
                            }
                        }
                    }
                }
        ...
        void CheckStatement(TreeNode parent)
        {
            TreeNode node = new TreeNode(Tokens.CheckDatabase);
            parent.AddChild(node);

            CheckDatabase(node);
        }

        void CheckDatabase(TreeNode parent)
        {
            ...
            if (token.LiteralValue == "DATABASE")
            {
                node = new TreeNode(Tokens.Keyword, "CHECK DATABASE");
                parent.AddChild(node);
                token = lex.GetToken();
            }
            else
            ...
            if (token.LiteralValue == "SYSTEM")
            {
                token = lex.GetToken();

                node = new TreeNode(Tokens.Keyword, "SYSTEM ONLY");
                parent.AddChild(node);
            }
        }
